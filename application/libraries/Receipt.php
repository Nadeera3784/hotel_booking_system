...
class Receipt{
	...
	public $searchVars        = array();
	public $detailVars	       = array();
	...
	//Set SESSION params
  public function set_request_rarams() {
  ...
  //Get advance_payment based on month
	public function advance_payment(){
	   
	   $month  = intval(substr($this->mysqlCheckInDate, 5, 2)) ;

	   $query = $this->ci->db->query("SELECT * FROM app_advance_payment WHERE month_num = ".$month);
  ...
  public function generate_booking_details() {
	  ...
	  for($i = 0; $i < $selectedRoomsCount; $i++){
			if($this->selectedRooms[$i] > 0){		
				$this->detailVars[$dvarsCtr] = $this->searchVars[$i]; 					
					$tmpTotalPrice = 0;
					$tmpTotalPrice2 = 0;
					...
					$child_flag2 = false;
					$query = $this->ci->db->query("SELECT distinct(`no_of_child`) FROM `app_room` WHERE `capacity_id`=".$this->detailVars[$dvarsCtr]['capacityid']." and `roomtype_id`=".$this->detailVars[$dvarsCtr]['roomtypeid']."");
		...
		if($this->ci->config_manager->config['conf_tax_amount'] > 0 &&  $this->ci->config_manager->config['conf_price_with_tax']==0){ 
			$this->roomPrices['totaltax'] = ($this->roomPrices['subtotal'] * $this->ci->config_manager->config['conf_tax_amount'])/100;
  ...
  public function generate_blocking_details() {
		$result = array();
	  $dvroomidsonly = "";
	  $selectedRoomsCount = count($this->selectedRooms);	
	  $dvarsCtr = 0;
